...
def emitir_declaracao(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            corpo_declaracao = [
                Paragraph(f"Declaramos para os devidos fins que <strong>{declaracao.docente.nome}</strong>, inscrito(a) sob o CPF nº <strong>{declaracao.docente.cpf}</strong>, exerceu atividades como tutor(a) do curso <strong>{declaracao.curso.nome}</strong>, na modalidade semi-presencial, nesta Escola de Saúde Pública do Rio Grande do Norte - ESPRN, instituição integrante da Rede de Escolas Técnicas do SUS - RETSUS e da Rede Nacional de Escolas de Saúde Pública - RedEscola, perfazendo a carga horária total de {declaracao.curso.carga_horaria} horas.", estilo_corpo),
            ]
